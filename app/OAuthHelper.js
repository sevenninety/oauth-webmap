define(["dojo/_base/lang", "dojo/_base/json", "dojo/_base/url", "dojo/cookie", "dojo/Deferred", "dojo/io-query", "esri/IdentityManager"], 
	...
	var OAuthHelper = {
		...
		popupCallbackPage : window.location.protocol + "//" + window.location.host + window.location.pathname.replace(/\/[^\/]+$/, "") + "/oauth-callback.html",
		...
		signIn : function() {
			...
			var authParameters = {
				...
				redirect_uri : this.popup ? this.popupCallbackPage : window.location.href.replace(/#.*$/, "")
			};
			...
			if (this.popup) {
				...
				window.open(authUrl, "esrioauth", "width=480,height=320,location=yes,status=yes,scrollbars=yes");
			} else {
		...
		parseFragment : function(url) {
			var urlObj = new Url(url), fragment = urlObj.fragment ? ioquery.queryToObject(urlObj.fragment) : null;
